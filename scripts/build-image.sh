...
# Note: VPP startup configs (startup-core.conf and startup-nat.conf) are
# now templates generated by configure-router.py with CPU-specific settings
...
cp "${CONFIG_DIR}/etc/systemd/system/incus-init.service" "${MOUNTPOINT}/etc/systemd/system/"
...
mkdir -p "${MOUNTPOINT}/etc/systemd/system/frr.service.d"
cp "${CONFIG_DIR}/etc/systemd/system/frr.service.d/after-vpp.conf" "${MOUNTPOINT}/etc/systemd/system/frr.service.d/"

# Helper scripts (static scripts)
# Note: incus-init.sh is now a template generated by configure-router.py
